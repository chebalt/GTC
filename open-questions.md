...
| 11 | ~~**Celum — asset migration approach**~~ | Actum (Michal + Art) + Aaron (GROHE) | **RESOLVED** — see Resolved section |
...
| Celum / asset migration | **RESOLVED 11 Apr 2026** — Craft assets taken as-is and re-uploaded to a CDN (likely Google Cloud Storage). No Celum involvement. During dev phase: use Craft URLs (`https://lc.training.grohe.this.work/assets/...`). Before launch: upload to CDN and swap URLs. KeyvisualUrl field (Single-Line Text) added to `_GtcBasePageTemplate` for all 3 page types. |
